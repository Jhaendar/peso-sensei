...
const fetchUserCategories = async (userId: string | undefined, type: 'income' | 'expense'): Promise<Category[]> => {
  ...
  return snapshot.docs.map(doc => {
    ...
    return {
      id: doc.id,
      ...data,
      createdAt: createdAtRaw instanceof Timestamp ? createdAtRaw.toDate() : new Date(createdAtRaw as any) // Handle potential string date
    } as Category;
    ...
    if (data.date) {
      try {
        ...
        if (!isNaN(parsedDate.valueOf())) {
          ...
          toast({ variant: "destructive", title: "AI Error", description: `AI returned an invalid date format: ${data.date}. Please set manually.` });
        }
      } catch (e) {
        toast({ variant: "destructive", title: "AI Error", description: `Could not parse date from AI: ${data.date}. Please set manually.` });
      }
    }

    if (availableCategories && data.category) {
        if (data.category.toLowerCase() === "other") {
            toast({ title: "AI Suggestion", description: `AI suggested "Other" category. Please select manually or add it if needed.`});
        } else {
            ...
            if (matchedCategory && matchedCategory.id) {
                ...
                toast({ title: "AI Suggestion", description: `AI suggested category: "${data.category}". Please select from the list or add it.` });
            }
        }
    }
        ...
        <CardContent className="px-4 pt-3 pb-3 sm:px-6 sm:pt-4">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2 sm:space-y-3">
              ...
              />
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground" disabled={form.formState.isSubmitting || !user || !db || isAIScanning}>
